...
; Edited by Jason www.codebot.de . 24.08.2012
; http://www.codebot.de/board77-autoit/board80-files-udfs/8291-ff-au3-f�r-firefox-6-�nderungen-aufgelistet
#cs Meine �nderungen
In Firefox 6 / 7 werden 2 Fenster gleichzeitig gestartet wenn man den Befehl _FFStart() nutzt:
=> Anpassung 1 in __FFStartProcess
Vorher:
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Die FF.au3 funktioniert nicht korrekt mit x64
=> Anpassung von __FFStartProcess
Vorher:
...
Nachher:
    ...
    If @error Then
        ...
        Return 0
       EndIf
	; !ACHTUNG! Der folgende String ist noch nicht an Problem 1 angepasst worden. !ACHTUNG!
    Local $sCommand = StringFormat('"%s" %s %s %s -repl %i %s', $sFFExe, $sNewWin, $sURL, $sNoRemote, $iPort, $sProfile)
    $PID = Run($sCommand)
Quelle: Der_Doc www.autoit.de
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Ab Firefox 4+ funktioniert der Hintergrundmodus nicht mehr!
=> Anpassung von __FFStartProcess
Vorher:
...
Nachher:
                ...
                BlockInput(0)
Quelle: misterspeed www.autoit.de
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Problem mit _FFLoadWait
=> Anpassung von _FFLoadWait
Vorher:
...
Nachher:
    ...
    __FFSendJavaScripts()
    Return SetError(__FFError($sFuncName, $_FF_ERROR_Timeout, "Can not check site status."), $iLoadingTime, 0)
Quelle: www.autoit.de
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Folgendes wurde NICHT ver�ndert, da die Funktion f�r die interne Fehlerverwaltung unver�ndert bleiben muss!
Zu Testzwecken kann dies aber genutzt werden um eine genauere Fehlerbeschreibung zu bekommen.
=> Anpassung von _FFCmd
Vorher:
        If $bTry And Not StringInStr(StringLower($sArg), "try") Then $sArg = "try{" & $sArg & "}catch(e){'_FFCmd_Err';};"
Nachher:
        If $bTry And Not StringInStr(StringLower($sArg), "try") Then $sArg = "try{" & $sArg & "}catch(e){e;};"
Quelle: Karill / Derpy Hooves @ www.codebot.de
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#ce
#Region Copyright
#cs
